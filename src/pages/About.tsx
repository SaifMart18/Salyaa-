    ...
    <div className="pt-48 pb-24 overflow-x-hidden">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <section className="mb-40">
          <div className={`flex flex-col lg:flex-row gap-20 items-start ${isRTL ? 'lg:flex-row-reverse' : ''}`}>
            <div className={`flex-[1.5] space-y-12 ${isRTL ? 'text-right' : 'text-left'}`}>
              ...
              <h1 className="text-7xl md:text-[110px] font-bold tracking-tighter leading-[0.85] text-white">
                {isRTL ? (
                  <>نبتكر للمستقبل، <br/><span className="text-brand-blue">نصمم</span> للتميز.</>
                ) : (
                  <>Innovating the future, <br/><span className="text-brand-blue">Designing</span> for excellence.</>
                )}
              </h1>
              
              <p className="text-2xl md:text-3xl text-white/60 leading-relaxed font-light max-w-3xl">
                {isRTL 
            ...
            <div className="flex-1 w-full grid grid-cols-1 sm:grid-cols-2 gap-6">
              {stats.map((stat, i) => (
                <motion.div 
                  ...
                  className="p-10 rounded-[40px] bg-white/[0.03] border border-white/5 hover:border-brand-blue/30 transition-all group"
                >
        ...
        <section className="py-32 border-t border-white/5">
          <div className={`flex flex-col lg:flex-row gap-24 items-center ${isRTL ? 'lg:flex-row-reverse' : ''}`}>
            <div className="flex-1 relative group">
              <motion.div 
                initial={{ opacity: 0, scale: 0.95 }}
                whileInView={{ opacity: 1, scale: 1 }}
                className="relative z-10 aspect-[4/5] rounded-[60px] overflow-hidden border border-white/10"
              >
                <img 
                  src={team[0].image} 
                  alt={team[0].name} 
            ...
            <div className={`flex-1 space-y-8 ${isRTL ? 'text-right' : 'text-left'}`}>
              ...
              <p className="text-2xl text-white/50 leading-relaxed italic border-l-4 border-brand-blue pl-8 py-4">
                "{team[0].bio}"
              </p>
